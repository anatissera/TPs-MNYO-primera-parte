...
interpol_x1_vehic1 = CubicSpline(mediciones_1_df.index, mediciones_1_df["x1"]) # uso los índices de las filas del DataFrame como los puntos de referencia para la interpolación
interpol_x2_vehic1 = CubicSpline(mediciones_1_df.index, mediciones_1_df["x2"])
...
def graficar_trayectoria_splines_3():
    ...
    plt.plot(interpol_x1_vehic1(points_to_evaluate_v1), interpol_x2_vehic1(points_to_evaluate_v1), label='Interpolación con Splines Cúbicos', color='red')
...
def graficar_error_abs():
    error_x1 = np.abs(ground_truth_df["x1"] - interpol_x1_vehic1(points_to_compare))
    error_x2 = np.abs(ground_truth_df["x2"] - interpol_x2_vehic1(points_to_compare))
...
def graficar_error_en_tiempo():
    error_x1 = np.abs(ground_truth_df["x1"] - interpol_x1_vehic1(ground_truth_df.index))
    error_x2 = np.abs(ground_truth_df["x2"] - interpol_x2_vehic1(ground_truth_df.index))
